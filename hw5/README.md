...
Netfilter为多种网络协议各提供了一套钩子函数。像有几个钓鱼台，在每个钓鱼台放了一个鱼钩(钩子函数)，把经过的数据包钓上来，然后根据自定义的规则，来决定数据包的命运：可以原封不动的放回IP协议，继续向上层递交；也可以进行修改，再放回IP协议；也可以直接丢弃。
...
4、filter规则过滤，网络地址转换后进入POSTROUTING阶段封包发出
