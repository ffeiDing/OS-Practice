...
def main():
	...
	while True:
		...
		stats_url = 'http://172.16.6.224:8888'
		stats_request = urllib.request.Request(stats_url)
		try:
			stats_reponse = urllib.request.urlopen(stats_request, timeout = 2)
		except (urllib.error.URLError, http.client.RemoteDisconnected, socket.timeout):
			print('[INFO] Master is not running on host 172.16.6.224')
		else:
			print('[INFO] Have found master on: 172.16.6.224')
			if last_master != 5:
				...
				args = ['/usr/local/bin/configurable-http-proxy', \
				'--default-target=http://172.16.6.224:8888', \
				'--ip=' + ip_addr, '--port=8888']
				last_pid = subprocess.Popen(args)
				last_master = 5
		...
		stats_url = 'http://172.16.6.213:8888'
		stats_request = urllib.request.Request(stats_url)
		try:
			stats_reponse = urllib.request.urlopen(stats_request, timeout = 2)
		except (urllib.error.URLError, http.client.RemoteDisconnected, socket.timeout) as e:
			print('[INFO] Master is not running on host 172.16.6.213')
		else:
			print('[INFO] Have found master on: 172.16.6.213')
			if last_master != 6:
				...
				args = ['/usr/local/bin/configurable-http-proxy', \
				'--default-target=http://172.16.6.213:8888', \
				'--ip=' + ip_addr, '--port=8888']
				last_pid = subprocess.Popen(args)
				last_master = 6
